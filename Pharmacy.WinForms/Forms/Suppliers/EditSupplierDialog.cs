...
internal sealed class EditSupplierDialog : Form
{
    ...
    public EditSupplierDialog(SupplierListItemView supplier) : this(supplier, AppServices.SupplierService)
    {
    }

    public EditSupplierDialog(SupplierListItemView supplier, SupplierService supplierService)
    {
    ...
    private void BuildUi(SupplierListItemView supplier)
    {
        ...
        _nameBox.Text = supplier.RawName;
        _contactBox.Text = supplier.ContactPerson == "غير متوفر" ? string.Empty : supplier.ContactPerson;
        _phoneBox.Text = supplier.PhoneNumber == "لا يوجد رقم" ? string.Empty : supplier.PhoneNumber;
        _addressBox.Text = supplier.Address == "—" ? string.Empty : supplier.Address;
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            var request = new UpdateSupplierApiRequest
            {
            ...
            var result = await _supplierService.UpdateSupplierAsync(_supplierId, request).ConfigureAwait(true);
            if (!result.Success)
            {
                SetStatus(result.ErrorMessage ?? "تعذر تحديث المورد.", true);
                MessageBox.Show(
                    this,
                    result.IsConnectionError
                        ? $"{result.ErrorMessage}{Environment.NewLine}تحقق من أن API يعمل على {ApiConfiguration.BaseUrl}"
                        : result.ErrorMessage ?? "تعذر تحديث المورد.",
                    "فشل الحفظ",
