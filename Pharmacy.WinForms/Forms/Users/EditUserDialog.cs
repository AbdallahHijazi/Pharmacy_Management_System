...
internal sealed class EditUserDialog : Form
{
    private readonly UserService _userService;
    private readonly UserListItemView _sourceUser;
    ...
    public EditUserDialog(UserListItemView sourceUser, UserService userService)
    {
    ...
    private void BuildUi()
    {
        ...
        layout.Controls.Add(_activeCheck, 0, 4);
        ...
        _saveButton = new GradientRoundedButton { Text = "حفظ التعديلات", IconGlyph = SegoeMdl2Icons.Save, Width = 170, Height = 46 };
    ...
    private async Task InitializeAsync()
    {
        ...
        var details = await _userService.LoadUserDetailsAsync(_sourceUser.Id).ConfigureAwait(true);
        var rolesResult = await _userService.LoadRolesAsync().ConfigureAwait(true);
        if (details is null)
        {
            ShowStatus("تعذر تحميل بيانات المستخدم.");
            return;
        }

        if (!rolesResult.Success || rolesResult.Roles.Count == 0)
        {
            ShowStatus(rolesResult.ErrorMessage ?? "تعذر تحميل الأدوار.");
            return;
        }

        _nameBox.Text = details.DisplayName;
        _emailBox.Text = details.Email;
        _phoneBox.Text = details.Phone;
        _activeCheck.Checked = details.IsActive;

        _roleCombo.DataSource = rolesResult.Roles.ToList();
        _roleCombo.DisplayMember = nameof(RoleListItemView.Name);
        _roleCombo.ValueMember = nameof(RoleListItemView.RoleId);
        _roleCombo.SelectedValue = details.RoleId;
        if (SessionManager.CurrentUser?.UserId == _sourceUser.Id)
        {
            _activeCheck.Enabled = false;
        }
    ...
    private async Task SaveAsync()
    {
        ...
        try
        {
            ...
            if (!result.Success)
            {
                ShowStatus(result.ErrorMessage ?? "تعذر تحديث المستخدم.");
                return;
            }
    ...
    private void ApplyThemeVisuals() => _saveButton.Invalidate();

    private static TextBox CreateTextBox(bool rtl)
    {
        return new TextBox
        {
